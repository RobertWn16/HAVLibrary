...
//#define USE_HAV_ENCODE

#ifdef USE_HAV_ENCODE
...
#else
winrt::com_ptr<ID3D11Device> pd3d11_dev;
winrt::com_ptr<IDXGIFactory2> pdxgi_fty;
winrt::com_ptr<IVideoSource> display_source;
winrt::com_ptr<ID3D11DeviceContext> pd3d11_ctx;
winrt::com_ptr<IDXGIDevice3> pdxgi_dev;
winrt::com_ptr<IDXGIAdapter> pdxgi_adapt;
winrt::com_ptr<IDemuxer> demx;
winrt::com_ptr<IDev> dev_nvidia;
winrt::com_ptr<IPacket> pckt;
winrt::com_ptr<IVideoOutput> vout;
...
void DecodeMain(IDecoder* dec, THREAD_PARAMS& par, VIDEO_SOURCE_DESC vsrc_desc) noexcept
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        hr = pdxgi_fty->CreateSwapChainForHwnd(pd3d11_dev.get(), par.hwnd, &dxgi_desc, NULL, NULL, &pdxgi_swpch);
        DXGI_RGBA rgba = { };
        rgba.a = 255;
...
void DecodeMainLoop(THREAD_PARAMS par)
{
    ...
    try {
        DEV_DESC dv_desc;
        dv_desc.vendor = NVIDIA;
        dv_desc.ordinal = 0;
        winrt::check_hresult(hav_instance->CreateDevice(IID_HAV_NVDev, dv_desc, dev_nvidia.put()));
...
int main(int argc, char** argv)
{
    ...
    if (SUCCEEDED(hr)) hr = pdxgi_fty->EnumAdapters(0, (IDXGIAdapter**)pdxgi_adapt.put());
