...
int main(int argc, char** argv)
{
    ...
    if (SUCCEEDED(hr)) hr = CreateDXGIFactory1(IID_IDXGIFactory2, (void**)&pdxgi_fty);
    ...
    if (SUCCEEDED(hr))
    {
        ...
        hr = pdxgi_fty->CreateSwapChainForHwnd(pd3d11_dev, hwnd, &dxgi_desc, NULL, NULL, &pdxgi_swpch);
        DXGI_RGBA rgba = { };
        rgba.a = 255;
    ...
    try {
        ...
        winrt::check_hresult(demx->VideoCapture("sony.mp4", source.put()));
    ...
    if (SUCCEEDED(hr))
    {
        ...
        pdxgi_swpch->ResizeBuffers(3, vsrc_desc.width, vsrc_desc.heigth, DXGI_FORMAT_B8G8R8A8_UNORM, 0);
    }
    ...
    while (true)
    {
        ...
        dec->Decode(nv_frame.get());
        nv_frame->ConvertFormat(HV_FORMAT_RGBA8, rgba_frame.get());
        rgba_frame->CommitResource();
